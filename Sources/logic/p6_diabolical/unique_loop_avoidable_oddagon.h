...
inline bool even_length_path_between_strict_nodes(
    const CandidateState& st,
    const int* strict_cells,
    int strict_count,
    const int* cell_to_node,
    ...
    while (qh < qt) {
        ...
        for (int p = p0; p < p1; ++p) {
            ...
            const int npar = 1 - par;
            const int nd = dep + 1;
            if (v == target_node && npar == 0 && nd >= min_edges) {
                return true;
            }
            int* vis = (npar == 0) ? vis0 : vis1;
            if (vis[v] != 0) continue;
            ...
            while (w2 != 0ULL) {
                ...
                for (int a = 0; a < neigh_cnt && !elim_pair_from_pivot; ++a) {
                    for (int b = a + 1; b < neigh_cnt && !elim_pair_from_pivot; ++b) {
                        if (even_length_path_between_strict_nodes(
                                st,
                                strict_cells,
                                strict_count,
                                cell_to_node,
